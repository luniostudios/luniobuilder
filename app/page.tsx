  ...
  return (
    <div className='bg-[#111114] min-h-screen justify-between flex flex-col'>
      ...
      <div className='text-white flex flex-row items-center gap-10 justify-center align-middle max-md:pt-20 max-md:flex-col max-lg:px-5'>
        <div className='flex w-1/2 flex-col gap-10 max-w-2xl max-lg:w-full'>
          ...
          <div className='flex flex-col gap-2'>
            <div className='flex flex-col sm:flex-row gap-4 max-lg:flex-row items-center'>
              <Link href={session ? "/dashboard" : "/pricing"} className='bg-linear-to-r from-[#1D976C] to-[#93F9B9] text-gray-800 font-bold py-4 px-6 rounded-lg max-md:text-sm'>
                {session ? 'Go to Dashboard' : 'Start Building Today'}
              </Link>
              <Link href="/documentation" className='text-white text-lg py-2 px-4 underline hover:text-gray-300 transition-colors font-bold underline-offset-4'>
                Documentation
              </Link>
            </div>
            <h1 className='flex w-full text-xs text-gray-400'>No credit card required. Free Forever.</h1>
          </div>
        </div>
