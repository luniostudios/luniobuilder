...
export const htmlToBuilderPages = (htmlContent: string): GeneratedPage[] => {
  const temp = document.createElement('div');
  temp.innerHTML = htmlContent;
  const pageWrappers = Array.from(temp.children).filter(child => child.hasAttribute('data-lunio-page'));

  if (pageWrappers.length === 0) {
    return [{ name: 'Home', slug: '/', elements: htmlToBuilderElements(htmlContent, null) }];
  }

  return pageWrappers.map((wrapper, index) => ({
    name: wrapper.getAttribute('data-lunio-page')?.trim() || (index === 0 ? 'Home' : `Page ${index + 1}`),
    slug: wrapper.getAttribute('data-lunio-slug')?.trim() || (index === 0 ? '/' : `/page-${index + 1}`),
    elements: Array.from(wrapper.children).map(child => nodeToBuilderElement(child, null)).filter((element): element is BuilderElement => Boolean(element)),
  }));
};
